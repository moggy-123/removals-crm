      ...
      {f.stages.map((st, idx) => {
        ...
        return (
          <Card key={st.id} style={{ background: "#FAFCFB" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <div style={{ flex: 1 }}><Field label="Date"><Input type="date" value={st.date} onChange={v => setStage(idx, "date", v)} /></Field></div>
              <div style={{ width: 120 }}><Field label="Time"><Input type="time" value={st.time} onChange={v => setStage(idx, "time", v)} /></Field></div>
            </div>
